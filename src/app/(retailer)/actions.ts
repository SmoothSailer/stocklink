...
export async function getPublicProducts() {
  ...
  const { data, error } = await supabase
    .from("products")
    .select(
      "*, wholesalers(id, name, location, phone, sales_rep_id, sales_reps(id, name, phone, whatsapp_phone, bio, avatar_url)), manufacturers(id, name, is_international, country_code), product_unit_options(*), product_media(*)"
    )
...
export async function getTrendingProducts() {
  ...
  const { data, error } = await supabase
    .from("products")
    .select("*, wholesalers(id, name, location), manufacturers(id, name, is_international, country_code), product_unit_options(*), product_media(*)")
    .eq("is_trending", true)
    .order("created_at", { ascending: false });
  if (error) throw new Error(error.message);
  return data;
